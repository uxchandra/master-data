<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-slate-800 leading-tight">Tambah Item Material</h2>
    </x-slot>
    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white shadow-sm rounded-xl border border-slate-200 overflow-hidden">
            ...
            <div class="p-6">
                <form method="POST" action="{{ route('materials.items.store') }}" class="space-y-5">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            ...
                            <x-text-input id="unique_no" name="unique_no" type="text" class="mt-1 block w-full font-mono"
                                :value="old('unique_no')" required autofocus />
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            ...
                            <x-text-input id="spec" name="spec" type="text" class="mt-1 block w-full" :value="old('spec')" />
                            <x-input-error :messages="$errors->get('spec')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="shape" :value="__('Shape')" />
                            <x-text-input id="shape" name="shape" type="text" class="mt-1 block w-full" :value="old('shape')" />
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            <div>
                                ...
                                <x-text-input id="blank_thick" name="blank_thick" type="number" step="0.001" class="mt-1 block w-full" :value="old('blank_thick')" />
                                <x-input-error :messages="$errors->get('blank_thick')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="blank_width" :value="__('Width')" />
                                <x-text-input id="blank_width" name="blank_width" type="number" step="0.001" class="mt-1 block w-full" :value="old('blank_width')" />
                                <x-input-error :messages="$errors->get('blank_width')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="blank_pitch" :value="__('Pitch')" />
                                <x-text-input id="blank_pitch" name="blank_pitch" type="number" step="0.001" class="mt-1 block w-full" :value="old('blank_pitch')" />
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            <div>
                                ...
                                <x-text-input id="cut_thick" name="cut_thick" type="number" step="0.001" class="mt-1 block w-full" :value="old('cut_thick')" />
                                <x-input-error :messages="$errors->get('cut_thick')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="cut_width" :value="__('Width')" />
                                <x-text-input id="cut_width" name="cut_width" type="number" step="0.001" class="mt-1 block w-full" :value="old('cut_width')" />
                                <x-input-error :messages="$errors->get('cut_width')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="cut_length" :value="__('Length')" />
                                <x-text-input id="cut_length" name="cut_length" type="number" step="0.001" class="mt-1 block w-full" :value="old('cut_length')" />
